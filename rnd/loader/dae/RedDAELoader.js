	...
	parseMesh = function (tRedDAELoader, redGL, rawData) {
		...
		meshList.forEach(function (mesh) {
			...
			mesh.querySelectorAll('polylist').forEach(function (pData, pDataIndex) {
				...
				tResultMesh['material'] = RedColorPhongMaterial(redGL)
				// 대상 메쉬를 결과메쉬에 추가
				...
				var aniIndex = 0
				var aniMax = aniInfo['Wolf_Skeleton_root_pose_matrix']['time'].length
				...
				var makeMatrix = function (list, target) {
					...
					for ( var k in aniInfo ) {
						if (controllerInfo[target] &&  aniInfo[k]['target'] == controllerInfo[target]['name'] ) {
							tAniMatrix = aniInfo[k]['matrix'][aniIndex];
							break
						}
					}
					if(controllerInfo[target]){
					list.push({
