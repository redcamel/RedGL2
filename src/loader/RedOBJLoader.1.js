...
(function () {
    ...
    setMaterial = function (redGL, v, materialInfo) {
        console.log(v)
        for (var k in v) {
            if (v[k]['use'] && v[k]['resultPosition'].length) {
                var tMaterial;
                console.log(v[k])
                console.log(v[k]['materialKey'], materialInfo[v[k]['materialKey']])
                tMaterial = materialInfo[v[k]['materialKey']] ? materialInfo[v[k]['materialKey']] : RedColorMaterial(redGL, '#00ff00')
                v[k]['mesh']['material'] = tMaterial
            }
        }
    }
    parser = function (target, redGL, data) {
        ...
        lines.forEach(function (line) {
                ...
                mtlLoader = RedMTLLoader(redGL, target['path'], line.split(' ')[1], function (v) {
                    ...
                    if (target['modelParsingComplete']) {
                        if (target['callback']) {
                            ...
                            setMaterial(redGL, info2, mtlLoader['parseData'])
            ...
            // 오브젝트 검색
                ...
                if (!currentMeshInfo) {
                    ...
                    info2[tName] = currentMeshInfo
                    currentObjectName = tName
                    if(firstRootInfo) firstRootInfo['use'] = false
                    else firstRootInfo = currentMeshInfo
                }
            }
            // 포지션 검색
        ...
        // console.log(lines)
        var newData = {}
        var resultMesh = target['resultMesh']
        var makeMesh;
