    ...
    parser = function (redGL, data) {
        ...
        lines.forEach(function (line) {
            ...
            if (regVertex.test(line)) {
                ...
                currentMeshInfo['points'][currentMeshInfo['points'].length] = [+tPosition[1], +tPosition[2], +tPosition[3]]
            ...
            // // TODO 노말 검색
                ...
                currentMeshInfo['normalPoints'][currentMeshInfo['normalPoints'].length] = [+tNormal[1], +tNormal[2], +tNormal[3]]
            ...
            //     var tUV;
            //     tUV = line.split(' ');
            //     currentMeshInfo['uv'].push(+tUV[1], +tUV[2])
            //     currentMeshInfo['uvPoints'][currentMeshInfo['uvPoints'].length] = [+tUV[1], +tUV[2]]
            ...
            // 인덱스 검색 1//1 1//1 1//1 v//n
                ...
                tData.forEach(function (v) {
                    ...
                    tIndex = +v[0] - 1
                    tNIndex = +v[2] - 1
                    tPoint = currentMeshInfo['points'][tIndex]
                    tNormalPoint = currentMeshInfo['normalPoints'][tNIndex]
                    if (currentMeshInfo['position'].length) max += 3
                    if (currentMeshInfo['normal'].length) max += 3
                    //
                    currentMeshInfo['index'].push(currentMeshInfo['resultInterleave'].length / max)
                    //
                    if (currentMeshInfo['position'].length) {
                    ...
                    if (currentMeshInfo['normal'].length) {
            ...
            // 인덱스 검색 1/1/1 1/1/1 1/1/1  v/uv/n
                ...
                tData.forEach(function (v) {
                    ...
                    tNIndex = +v[2] - 1
                    tPoint = currentMeshInfo['points'][tIndex]
                    tUVPoints = currentMeshInfo['uvPoints'][tUVIndex]
                    tNormalPoint = currentMeshInfo['normalPoints'][tNIndex]
                    if (currentMeshInfo['position'].length) max += 3
                    if (currentMeshInfo['normal'].length) max += 3
                    if (currentMeshInfo['uv'].length) max += 2
                    //
                    currentMeshInfo['index'].push(currentMeshInfo['resultInterleave'].length / max)
                    //
                    if (currentMeshInfo['position'].length) {
        ...
        for (var k in info) {
            ...
            if (info[k]['position'].length) {
                ...
                // 인터리브 버퍼 생성
                var tInterleaveInfo = []
                if (info[k]['position'].length) tInterleaveInfo.push(RedInterleaveInfo('aVertexPosition', 3))
                if (info[k]['normal'].length) tInterleaveInfo.push(RedInterleaveInfo('aVertexNormal', 3))
                if (info[k]['uv'].length) tInterleaveInfo.push(RedInterleaveInfo('aTexcoord', 2))

                newData[k]['interleaveBuffer'] = RedBuffer(
                    redGL,
                    k + '_interleave',
                    new Float32Array(info[k]['resultInterleave'].length ? info[k]['resultInterleave'] : info[k]['position']),
