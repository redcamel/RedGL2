...
RedGL(document.createElement('canvas'), function (v) {
	...
	redSuite(
		...
		redGroup(
			...
			redTest("인자확인 : 생성키 중복 방지확인", function (unit) {
				try {
			...
			redTest("인자확인 : scene이 RedScene이 아닐떄", function (unit) {
				try {
			...
			redTest("인자확인 : camera가 RedCameara가 아닐떄", function (unit) {
				try {
		...
		redGroup(
			"setSize",
			redTest("setSize : Number, Number", function (unit) {
				var t0;
		...
		redGroup(
			"setLocation",
			redTest("setLocation : Number, Number", function (unit) {
				var t0;
