...
RedGL(document.createElement('canvas'), function (v) {
    var tRedGL = this;
    redSuite(
        "RedParticleColorMaterial 테스트",
        redGroup(
            "RedParticleColorMaterial( redGL )",
            redTest("성공테스트 : 기본 생성 테스트", function (unit, title) {
                try {
                    RedParticleColorMaterial(tRedGL);
                    unit.run(true)
                } catch (error) {
                    console.log('///////////////////////////////////////////////////////////')
                    console.log(title, '\n', error)
        ...
        redGroup(
            "(RedParticleColorMaterial Instance).<b>alpha</b> = value",
            redTest("실패테스트  : 생성인자 반영되는지 체크 : 숫자만 허용하는지", function (unit, title) {
                try {
                    var t0 = RedParticleColorMaterial(tRedGL);
                    t0.alpha = 'failTest'
                } catch (error) {
                    console.log('///////////////////////////////////////////////////////////')
                    console.log(title, '\n', error)
