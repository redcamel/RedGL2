...
RedGL(document.createElement('canvas'), function (v) {
	...
	redSuite(
		...
		redGroup(
			...
			redTest("addView - RedView Instance 만 허용하는지 체크", function (unit) {
				try {
			...
			redTest("addView - RedView Instance 만 허용하는지 체크 : 숫자입력시 에러확인", function (unit) {
				try {
					tWorld.addView(1)
					unit.run(true)
				} catch (error) {
			...
			redTest("addView - RedView Instance 만 허용하는지 체크 : 문자입력시 에러확인", function (unit) {
				try {
					tWorld.addView('문자')
					unit.run(true)
				} catch (error) {
		...
		redGroup(
			...
			redTest("getView : 키는 문자열만 허용", function (unit) {
				try {
					tWorld.getView(1)
					unit.run(true)
				} catch (error) {
			...
			redTest("getView : 존재하는 키로 검색할경우", function (unit) {
				unit.run(tWorld.getView("test_1")['key'])
			}, 'test_1'),
			redTest("getView : 존재하는 키로 검색할경우2", function (unit) {
				unit.run(tWorld.getView("test_2")['key'])
			}, 'test_2'),
			redTest("getView : 존재하지 않는 키로 검색할경우", function (unit) {
				unit.run(tWorld.getView("없는키"))
			}, undefined)
		),
		redGroup(
			"RedView 관리 확인 - hasView ",
			redTest("hasView : 키는 문자열만 허용", function (unit) {
				try {
					tWorld.hasView(1)
					unit.run(true)
				} catch (error) {
			...
			redTest("hasView : 존재하는 키로 검색할경우", function (unit) {
				unit.run(tWorld.hasView("test_2"))
			}, true),
			redTest("hasView : 존재하지 않는  키로 검색할경우", function (unit) {
				unit.run(tWorld.hasView("없는키"))
			}, false)
		),
		redGroup(
			"RedView 관리 확인 - delView ",
			redTest("delView : 키는 문자열만 허용", function (unit) {
				try {
					tWorld.delView(1)
					unit.run(true)
				} catch (error) {
			...
			redTest("delView : 존재하는 키로 삭제시도", function (unit) {
				tWorld.delView("test_1")
				unit.run(tWorld.getView("test_1"))
			}, undefined),
			redTest("hasView : 잘 삭제되었는지 확인", function (unit) {
