...
RedGL(document.createElement('canvas'), function (v) {
    var tRedGL = this;
    redSuite(
        "RedDirectionalLight Test",
        redGroup(
            "RedDirectionalLight( redGL, hexColor, alpha )",
        ...
        redGroup(
            "RedDirectionalLight( <b>redGL</b>, hexColor, alpha )",
        ...
        redGroup(
            "RedDirectionalLight( redGL, <b>hexColor</b>, alpha )",
            redTest("실패테스트 : hexColor - #xxxxxx or #xxx 만 허용 - 1", function (unit, title) {
                try {
                    RedDirectionalLight(tRedGL, 1);
                    unit.run(true)
                } catch (error) {
                    console.log('///////////////////////////////////////////////////////////')
                    console.log(title, '\n', error)
            ...
            redTest("실패테스트 : hexColor - #xxxxxx or #xxx 만 허용 - '#2233'", function (unit, title) {
                try {
                    RedDirectionalLight(tRedGL, '#2233');
                    unit.run(true)
                } catch (error) {
                    console.log('///////////////////////////////////////////////////////////')
                    console.log(title, '\n', error)
            ...
            redTest("성공테스트 : hexColor - #556677", function (unit, title) {
                var t0 = RedDirectionalLight(tRedGL, '#556677');
                unit.run(t0['color'])
            }, '#556677'),
            redTest("성공테스트 : hexColor - #fff", function (unit, title) {
                var t0 = RedDirectionalLight(tRedGL, '#fff');
                unit.run(t0['color'])
            }, '#fff'),
            redTest("성공테스트 : hexColor - #00ff00", function (unit, title) {
                var t0 = RedDirectionalLight(tRedGL, '#00ff00');
                unit.run(t0['_lightColor'][0] + '_' + t0['_lightColor'][1] + '_' + t0['_lightColor'][2])
            }, '0_1_0')
        ),
        redGroup(
            "RedDirectionalLight( redGL, hexColor, <b>alpha</b> )",
            ...
            redTest("실패테스트 : 숫자만 허용하는지", function (unit, title) {
                try {
                    RedDirectionalLight(tRedGL, '#fff', 'test');
                } catch (error) {
                    console.log('///////////////////////////////////////////////////////////')
                    console.log(title, '\n', error)
                    unit.run(false)
        ...
        redGroup(
            ...
            redTest("성공테스트 : 숫자만 허용", function (unit, title) {
                try {
                    var t0 = RedDirectionalLight(tRedGL);
                    t0['intensity'] = 1
                    unit.run(true)
